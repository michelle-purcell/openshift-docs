...
. Open the *{rhl-advisor}* tab of the cluster.
+
If the upload was successful, the tab displays one of the following:
+
* *Your cluster passed all recommendations*, if the {rhl-advisor-service} did not identify any issues.

* A list of issues that the {rhl-advisor-service} has detected, prioritized by risk (low, moderate, important, and critical).
